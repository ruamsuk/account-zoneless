...
@Component({
  ...
  template: `
    <main class="container mx-auto p-4 md:p-8">
      ...
      <!-- Filter Controls -->
      <div (click)="onContentClick()" class="mb-6 p-4 bg-white rounded-xl shadow-md dark:bg-gray-800">
        <form [formGroup]="dateRangeForm" (ngSubmit)="generateReport()"
          ...
          <div>
            <button type="submit" [disabled]="dateRangeForm.invalid" class="btn-primary w-full">แสดงรายงาน</button>
          </div>
        </form>
      </div>
...
export class BloodPeriod {
  ...
  shouldCloseDatePickers = signal(false);
  ...
  async generateReport(): Promise<void> {
    ...
    try {
      ...
      const results = await this.bpService.getTransactionsByDateRange(startDate, endDate);
      this.transactions.set(results);
    } catch (error) {
      console.error('Error fetching transactions by date range:', error);
      this.toastService.show('Error', 'ไม่สามารถดึงข้อมูลได้: ' + error, 'error');
  ...
  onContentClick(): void {
    this.shouldCloseDatePickers.set(true);
  }
  ...
  async onSave(dataToSave: any): Promise<void> {
    ...
    try {
      ...
      this.closeModal();
      await this.generateReport(); // <-- โหลดข้อมูลใหม่หลังบันทึก
    } catch (error) {
  ...
  async onDelete(item: BloodPressure): Promise<void> {
    ...
    if (confirmed && item.id) {
      ...
      try {
        ...
        await this.generateReport(); // <-- โหลดข้อมูลใหม่หลังลบ
      } catch (error) {
